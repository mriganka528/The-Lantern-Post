...
export class NotificationsService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit() {
    if (this.enabled) { this.timer = setInterval(() => { void this.dispatch().catch(() => {}); }, 30_000); this.timer.unref(); }
  }
  onModuleDestroy() { if (this.timer) clearInterval(this.timer); }
  private async owner(subject: string) {
    const owner = await this.prisma.user.findUnique({ where: { authProviderId: subject }, select: { id: true } });
  ...
  async register(subject: string, input: PushRegistration) {
    ...
    await this.prisma.pushToken.upsert({ where: { token: input.token }, create: { userId: owner.id, ...input }, update: { userId: owner.id, platform: input.platform, createdAt: new Date() } });
    ...
    try {
      ...
      for (const job of jobs) {
        ...
        try {
          ...
          const valid = letterDelivery ? job.letterId && await this.prisma.letter.findFirst({ where: { AND: [privateLetters(job.userId), { id: job.letterId, recipientId: job.userId, readAt: null }] }, select: { id: true } }) :
          ...
          const response = await fetch('https://exp.host/--/api/v2/push/send', { method: 'POST', redirect: 'error', signal: AbortSignal.timeout(5000),
            ...
            body: JSON.stringify(tokens.map(t => ({ to: t.token, title: 'Lantern Post', body: letterDelivery ? 'A sealed letter is waiting at your palace gate.' : job.kind === 'FRIEND_REQUEST' ? 'A new invitation is waiting at your palace gate.' : 'A new gate has opened in your circle of friends.',
              sound: 'default', channelId: 'palace-invitations', ttl: 3600, data: { type: job.kind, eventId: job.id, ownerId: job.userId, ...(letterDelivery ? { letterId: job.letterId, screen: 'inbox' } : { requestId: job.requestId, screen: 'friends' }) } }))),
          });
