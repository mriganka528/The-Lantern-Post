...
export function DeliveryJourney({ recipient, preset, courier, preview = false, alreadyDelivered = false, onFinish }: { recipient: LetterRecipient; preset: LetterPreset; courier?: CharacterKey; preview?: boolean; alreadyDelivered?: boolean; onFinish: () => void }) {
  ...
  useEffect(() => {
    ...
    const walk = Animated.timing(progress, { toValue: 1, duration: reduced ? 0 : 6800, easing: Easing.linear, useNativeDriver: native });
  ...
  return <StoryShell chapter={preview ? 'A GLIMPSE OF THE JOURNEY' : 'CARRIED THROUGH THE CLOUDS'} actions={<TextAction label={preview ? 'Back to my letter' : 'My palace'} onPress={onFinish} />}>
    ...
    <View testID="delivery-journey">
      <PalaceScene characterKey={recipient.characterKey ?? 'fox-lantern'} paused={complete || reduced} worldOverlay={<View pointerEvents="none" onLayout={() => setLaidOut(true)} style={StyleSheet.absoluteFill}>
        <View style={styles.gateway}><View style={styles.gatewayLight} /><Text style={styles.gateName}>@{recipient.username}</Text>
    ...
    <Text accessibilityLiveRegion="polite" style={styles.caption}>{preview ? complete ? 'A glimpse of the path. Your letter is still yours.' : 'A little walk across the river, towards a familiar gate…' : complete ? 'Delivered privately. Only your two palaces can open this letter.' : 'Through the garden. Across the river. Into a friend’s keeping.'}</Text>
...
const styles = StyleSheet.create({
  ...
  gateName: { position: 'absolute', left: -18, right: -18, top: -25, textAlign: 'center', fontFamily: serif, color: '#6C5737', fontSize: 14 }, door: { position: 'absolute', top: 9, bottom: 0, width: 74, backfaceVisibility: 'hidden' }, fill: { position: 'absolute', width: '100%', height: '100%' }, shadow: { position: 'absolute', left: 35, top: 143, width: 70, height: 10, backgroundColor: 'rgba(76,65,44,.18)', borderRadius: 40 }, caption: { color: mutedInk, fontFamily: serif, fontStyle: 'italic', fontSize: 18, lineHeight: 27, textAlign: 'center', marginVertical: 23 },
});
