...
export function LanternMark({ size = 34, color = gold }: { size?: number; color?: string }) {
  return <Image source={require('../../assets/storybook/lantern.png')} style={{ width: size, height: size * 1.2, tintColor: color }} resizeMode="contain" accessible={false} />;
}
export function Flourish({ width = 180, color = gold }: { width?: number; color?: string }) {
  return <Image source={require('../../assets/storybook/flourish.png')} style={{ width, height: 24, tintColor: color }} resizeMode="contain" accessible={false} />;
}
const icons = {
...
export function StoryIcon({ kind, size = 24, color = gold }: { kind: keyof typeof icons; size?: number; color?: string }) {
  return <Image source={icons[kind]} style={{ width: size, height: size, tintColor: color }} resizeMode="contain" accessible={false} />;
}
