...
export function PalaceBell({ disabled = false, beforeOpen }: { disabled?: boolean; beforeOpen?: () => boolean }) {
  ...
  if (!bell) return null;
  return <Pressable testID="palace-notification-bell" accessibilityRole="button" accessibilityLabel={`Notifications${bell.unseen ? `, ${bell.unseen} unseen` : ', all seen'}`} accessibilityHint="Open recent letters, messages and invitations" accessibilityState={{ disabled }} disabled={disabled}
