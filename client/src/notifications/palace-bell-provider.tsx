...
function AccountBell({ ownerId, getToken, onOpen, chatPeerId, children }: Props) {
  ...
  const events = useMemo(() => (query.data?.events ?? []).filter(isBellArrival).slice().reverse(), [query.data]);
  ...
  }, [active, chatPeerId, events, store, cache, key]);
  ...
  const openBell = useCallback(() => { dismissPalaceArrival(ownerId); setOpen(true); void cache.invalidateQueries({ queryKey: key }); }, [cache, key, ownerId]);
  ...
  return <BellContext.Provider value={value}>{children}{open && active && <StoryDialog title="The palace bells" onClose={() => setOpen(false)}>
    ...
    <Text style={s.body}>Recent letters, messages and invitations from the past seven days.</Text>
    ...
    {events.map(event => { const copy = arrivalCopy(event); const isNew = event.alert && !seen.has(event.id); return <Pressable key={event.id} testID={`bell-notice-${event.id}`} accessibilityRole="button" accessibilityLabel={`${copy.title}${isNew ? ', new' : ''}. ${copy.action}`} onPress={() => { store.markSeen([event.id]); setOpen(false); onOpen(eventDestination(event)); }} style={({ pressed }) => [styles.notice, isNew && styles.newNotice, pressed && { backgroundColor: '#E8D8B5' }]}>
    ...
    {saved.error && <Text role="alert" style={s.body}>Which notices you have seen could not be saved on this device. Try marking them again.</Text>}
...
const styles = StyleSheet.create({
  ...
  noticeHeading: { flexDirection: 'row', alignItems: 'center', gap: 9 }, title: { fontFamily: serif, fontSize: 19, color: '#5C472E', flexShrink: 1 },
