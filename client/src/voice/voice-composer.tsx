...
function StoredVoicePlayer({ ownerId, clip }: { ownerId: string; clip: VoiceClip }) {
  ...
  useEffect(() => { let active = true; let release: (() => void) | undefined;
    void voiceStorage.playback(ownerId, clip).then(result => { if (!active) result.release(); else { release = result.release; setSource(result.uri); } }).catch(() => { if (active) setError(true); });
...
export function VoiceComposer({ ownerId, draft, clip, caption, clearing, onBusy, onRecording }: { ownerId: string; draft: DraftController; clip: VoiceClip | null; caption: string; clearing: boolean; onBusy: (busy: boolean) => void; onRecording?: (recording: boolean) => void }) {
  ...
  return <View style={styles.composer}>
    ...
    {discard && <StoryDialog title="Remove this recording?" onClose={() => setDiscard(false)}><Text style={s.body}>This removes the recording from this device. You can record a fresh take afterward.</Text><StoryButton label="Remove recording" onPress={() => { if (draft.discardVoice()) setDiscard(false); }} /><StoryButton label="Keep recording" secondary onPress={() => setDiscard(false)} /></StoryDialog>}
  </View>;
}
