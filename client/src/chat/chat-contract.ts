...
export interface ChatTransport {
  ...
  sync?(peerId: string, ids: string[], signal?: AbortSignal): Promise<ChatMessage[]>;
  remove?(messageId: string, scope: 'self' | 'everyone', signal?: AbortSignal): Promise<void>;
}
...
export function readChatPage(value: unknown, peerId: string, maximum = 50): ChatPage {
  if (!value || typeof value !== 'object') throw new ChatFault('connection'); const p = value as ChatPage;
  if (!p.peer || p.peer.id !== peerId || !/^[a-z0-9_]{3,24}$/.test(p.peer.username) || !Array.isArray(p.messages) || p.messages.length > maximum || !Number.isSafeInteger(p.cursor) || p.cursor < 0 || p.before !== null && (!Number.isSafeInteger(p.before) || p.before < 1)) throw new ChatFault('connection');
  let last = 0;
  const messages = p.messages.map(message => {
    ...
    if (message.removed !== undefined && (message.removed !== true || message.text !== 'This message was unsent.')) throw new ChatFault('connection');
    last = message.sequence; return { id: message.id, sequence: message.sequence, side: message.side, text: message.text, createdAt: message.createdAt, ...(message.removed ? { removed: true as const } : {}) };
  });
...
export function mergeMessages(current: ChatMessage[], incoming: ChatMessage[]) {
  const byId = new Map(current.map(message => [message.id, message])); incoming.forEach(message => { if (!byId.get(message.id)?.removed || message.removed) byId.set(message.id, message); });
