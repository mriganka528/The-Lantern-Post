...
export function privateKeyOwner(key: string): string | null {
  for (const prefix of ['lantern-draft-v1-','lantern-letter-choice-v1-','lantern-events-v1-','lantern-alerts-v1-','lantern-bell-seen-v1-']) if (key.startsWith(prefix)) return decodeURIComponent(key.slice(prefix.length));
