...
import { pauseVoicePlayback } from '../src/voice/playback-registry';
import { DiagnosticsSession } from '../src/diagnostics/diagnostics-session';
import { AccountSession } from '../src/account/account-session';
import { ClosedAccountCleanup } from '../src/account/closed-account-cleanup';
import { WelcomeNotificationSession } from '../src/notifications/welcome-session';
  ...
  return (
    ...
    <ClerkProvider publishableKey={env.clerkPublishableKey} tokenCache={tokenCache} __experimental_disableNativeClientSync>
      ...
      <View style={{ flex: 1, backgroundColor: '#FAF4E8' }}>
        <SessionRoutes />
        <ClosedAccountCleanup />
        <WelcomeNotificationSession />
        <ClerkCaptcha />
      </View>
    </ClerkProvider>
  );
...
function SessionRoutes() {
  ...
  useEffect(() => { pauseVoicePlayback(); }, [pathname]);
  ...
  return (
    <SessionDataProvider key={cacheKey}>
      <AccountSession key={cacheKey}><NotificationSession enabled={Boolean(isSignedIn) && pathname !== '/oauth-callback'}>
      <DiagnosticsSession><Stack screenOptions={{ headerShown: false, contentStyle: { backgroundColor: '#FAF4E8' } }}>
        ...
        <Stack.Protected guard={Boolean(isSignedIn)}>
          ...
          <Stack.Screen name="infinity" />
        </Stack.Protected>
      </Stack></DiagnosticsSession></NotificationSession></AccountSession>
    </SessionDataProvider>
  );
}
